...
VertexArray::VertexArray(std::vector<glm::vec3> _positions, std::vector<glm::vec2> _texCoords) : dirty(false)
{
	...
	if (!_positions.empty() && !_texCoords.empty() && _positions.size() == _texCoords.size())
	{
		for (int i = 0; i < _positions.size(); i++)
		{
			positionBuffer->add(_positions.at(i));
			texCoordBuffer->add(_texCoords.at(i));
		}
	}
...
VertexArray::VertexArray(std::string path, std::shared_ptr<std::vector<glm::vec3>> _modelPositions) : dirty(false)
{
	glGenVertexArrays(1, &id);
	...
	while (!file.eof())
	{
		...
		if (splitLine.at(0) == "v")
		{
			...
			_modelPositions->push_back(glm::vec3(
				atof(splitLine.at(1).c_str()),
				atof(splitLine.at(2).c_str()),
				atof(splitLine.at(3).c_str()))); // push back the vertices of the model into a passed vector
		}
		else if (splitLine.at(0) == "vt")
		{
			if (!texCoordBuffer) texCoordBuffer = new VertexBuffer();
